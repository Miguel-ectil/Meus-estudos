...
export const useEstampagemService = () => {
	...
	// Está sendo usado
	const updateFile = async (files: any): Promise<any> => {
		const url: string = `/docsAe/update`
		const response: AxiosResponse<any> = await httpClientDJango.post(url,files);
		return response.data
	};
...
{/*--------------------------------------------*/}
	...
	const cadastrarAe = async (data:any): Promise<any> => {
		const url:string = `${Url}/create-ae`;
		const response:AxiosResponse<any> = await httpClientDJango.post(url, data);
		return response.data
	}

	const cancelaEstampagem = async (data:any): Promise<any> => {
		const url:string = `${Url}/cancelar-estampagem`;
		const response:AxiosResponse<any> = await httpClientDJango.post(url, data);
		return response.data;
	}

	const registraEstampagem = async (data:any): Promise<any> => {
